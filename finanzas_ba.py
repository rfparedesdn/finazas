...
st.set_page_config(page_title="Gastos BA", page_icon="💰")
...
# Limpieza automática de la llave
...
# Credenciales (Siempre en inglés)
creds_dict = {
...
try:
    ...
    # Formulario básico para probar
    with st.form("registro"):
        item = st.text_input("¿Qué compraste?")
        precio = st.number_input("Monto ($)", min_value=0)
        if st.form_submit_button("Guardar"):
            sheet.append_row([item, precio])
            st.balloons()
            st.write(f"¡Listo! Se guardó {item}")
